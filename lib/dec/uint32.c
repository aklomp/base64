/* If we have native uint32's, pick off 4 bytes at a time for as long
 * as we can, but make sure that we quit before seeing any == markers
 * at the end of the string. Also, because we write a zero at the end
 * of the output, ensure that there are at least 2 valid bytes of
 * input data remaining to close the gap. 4 + 2 + 2 = 8 bytes: */
while (srclen >= 8)
{
