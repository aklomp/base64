...
// x86: SSE4.2: runtime check whether the codec is compiled in and compatible.
static inline bool
x86_sse42_codec_runnable (const unsigned int max_level)
{
	if (max_level < 1U) {
		return false;
	}

#if defined (BASE64_X86_SIMD) && HAVE_SSE42
	uint32_t eax, ebx, ecx, edx;
